...
from gtgui.shiftstats import GUIShiftStats
...
class GUIShiftBeep(GenericGUIShiftBeep, ShiftBeep):
    ...
    LOOP_FUNCS = ShiftBeep.LOOP_FUNCS + [
         ...
         'loop_update_vmax',    #set vmax in gui
    ...
    def init_gui_buttonframe(self):
        ...
        self.shiftstats = GUIShiftStats(frame, config)
    ...
    def init_gui_grid_buttonframe(self):
        ...
        if config.shift_stats_active:
            self.shiftstats.grid(row=0, column=2)
        
        if config.fuel_stats_active:
    ...
    def init_gui_grid(self):
        ...
        self.buttonframe.grid( row=row+1, column=4, rowspan=2, columnspan=4)    
        self.vmaxdata.grid(    row=row+2, column=0)
    ...
    def loop_update_vmax(self, gtdp):
        self.vmaxdata.update(gtdp)
